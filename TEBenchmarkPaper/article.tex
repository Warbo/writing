...
providing a direct comparison of these
...
like a tool to produce for those definitions. Each tool is run and their outputs
are compared to the ground truth. To score 100\% on precision and recall, all of
the conjectures which appear in the ground truth must be generated, and nothing
else:
...
We can summarise runtimes by choosing their median, as this is more robust
...
can sum their numerators and denominators to get the \emph{ratio of the
  averages}: this normalises the size of the generated and ground truth sets,
letting us model precision and recall as simple Bernoulli processes.
...
normally distributed (unlike, say, the z-test or t-test). In our application we
follow a variant of the Speedup-Test protocol of Touati, Worms and
Briais~\cite{touati2013speedup}; the differences are our use of the paired form
of the Wilcoxon test, and the fact that each run was performed on a different
sample (to reduce sampling bias, at the expense of accuracy).
...
compare the number of wanted and unwanted conjectures generated by each tool.
...
benchmark as our ground truth corpus.
...
\item Partial functions cause problems during exploration, since they can throw
  ...
  (for example $\Head(\Nil)$), hence preventing IsaCoSy from ever generating
  them.
\end{itemize}
...
These totals were tested for independence using Boschloo's test, assuming a
binomial distribution, and gave an p-value of 0.111. With our (arbitrary) choice
of $\alpha = 0.05$, this is not significant enough to reject the null hypothesis
that their precision is equal.
...
of recall by simply generating fewer conjectures. IsaCoSy's significantly lower
recall indicates that it may be overly-conservative in this way, compared to
QuickSpec.
...
%   theorem statements, which is enough to fully exercise current MTE tools.
...
% although MTE tools could .

% To provide a more robust comparison of these two tools, we also applied a
% paired difference test: measuring, for each sampled theory, the difference in
% time, precision and recall between the two tools.
...
as Colton's metrics in table~\ref{table:colton} which are proposed based largely
on a computer scientist's ``intuition'' and argumentation about why a metric
would be important (and previous use of such metrics, and - in the case of
...
allows us to side-step this philosophical quagmire to produce concrete,
measurable values; it is also a core weakness, as there are compelling reasons
to refute it.
...
envisaged by Lenat (or McCarthy's earlier
``advice taker''~\cite{McCarthy_Programs59}). Analogous benchmarks could be
designed for such long-running, potentially interactive programs, but that
