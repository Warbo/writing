...
and regular competitions to prove as much as possible, as fast as possible.
These problem sets are an opportunity for theory exploration, as their
definitions and theorems can be used as a corpus, in the same way that Isabelle
libraries have been used in the past.

Some problem sets are more amenable for this purpose than others. The most
suitable are those which meet the following criteria:

\begin{itemize}
\item For each problem, there should be a clear separation between the theorem
  to be proved and the definitions involved, such that the two can be easily and
  meaningfully separated. This rules out problem sets like SMTLIB, where many
  problems \emph{declare} a function, but leave its behaviour \emph{implicit} in
  the logical structure of the theorem statement.
...
\item The problem set should be relevant to the desired domain. In our case, we
  ...
  first-order languages/logics.
\item Since it will act as our ground truth, the problem set should ideally
  contain \emph{every} ``interesting'' conjecture involving its included
...
Once a theorem proving problem set has been chosen, we must separate the
definitions referenced by the theorems from the theorem statements themselves.
The former will be used as input to the ATE systems under evaluation, whilst the
latter form a ground truth corpus against which to compare the ATE output.
...
We form the ground truth as above: selecting all applicable theorems
from the corpus, which is now guaranteed to be non-empty.
...
In our experiments we have found that memory usage is also important, but rather
than complicating our analysis with an extra dimension, we instead set a limit
(as large as practical) and terminate program runs which cross it.~\footnote{We
  could treat the hardware's RAM capacity as our limit, allow the operating
  system to swap any overflow to disk, and wait for the inevitable slowdown to
  trigger the timeout. This requires no work to implement, but wastes a lot of
  time waiting for runs which could otherwise be terminated early.}

This approach process produces a single runtime, precision and recall value for
each sample. We propose two methods for analysing this data: \emph{summarising}
the performance of an ATE system, or \emph{comparing} the performance of
\emph{two} ATE systems.

\subsection{Summarising}

 - Giving each run a different sample is important for getting independent
   values which test the generality of the system. This is preferable to e.g.
   timing each sample multiple times.

 - What sorts of aggregate data to report: median vs mean, variance, quartiles,
   etc.
...
comparison based on them would have very low statistical power.

To produce more salient results, we should compare the data points directly, for
example with a \emph{paired difference test}: for each individual sample, we
find the \emph{difference} between the two systems' data \emph{for that sample}.
We then aggregate these differences, to determine whether one system performs
significantly better or worse than the other.

 - Good because: handles the variance of results well
 - Good because: approximates a normal distribution, making meaningful statistics easier
 - Good because: conservative in the face of timeouts

 - Question: do we use relative difference, e.g. [ 1.0:1.2, 1.0:1.5, ...], or do
   we use absolute differences, e.g. [100s:120s, 1s:1.5s, ...]?
