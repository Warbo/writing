...
for samples sized between 1 and 100, for an hour each. Most QuickSpec runs
...
perform well beyond this, and we followed the statistical ``rule of thumb'' of
picking 30 samples for each size\footnote{Only 29 samples of size 2 were
  analysed since one was a duplicate, due to the constraint that samples contain
  at least one theorem's dependencies.}. This gave $20$ sizes $\times 30$
samples $\times 2$ tools $= 1200$ runs. To keep the total time down to a few
days we chose a timeout of 300 seconds, since that covered most of the
...
another. The main reason is to avoid second-guessing the ground truth: the
theorems' authors chose to state them that way, and it is not our place to
decide whether an alternative form is ``just as good'' (for example, a more
general statement is more widely applicable, but it might also be harder to
understand).
...
The following issues had to be overcome for the benchmarking to be successful:

\begin{itemize}
  polyrec can't be represented
  constructors were being included
  proofs had to be skipped
  partial functions needed adding to the constraints
\end{itemize}
...
early due to running out of memory).

\subsection{Comparison}

We compare the performance of QuickSpec and IsaCoSy using a similar protocol to
the Speedup-Test of Touati, Worms and Briais~\cite{touati2013speedup}. We differ
by timing a wider variety of inputs: running many samples once, rather than a
few samples many times and taking the medians. Since each sample is run through
both tools, we can also gain statistical power by using the \emph{paired} form
