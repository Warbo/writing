...
output and ground-truth counts \emph{before} taking their ratios (the
\emph{ratio of the averages}). The former estimates the precision and recall we
can expect from any \emph{particular} run, which is of direct relevance to users
of these tools. The latter estimates the proportion of discoveries that a tool
can expect to find out of 'all possible', i.e. assigning higher weight to
performance in theories which have more conjectures available to be discovered;
whilst this is of theoretical interest, it is a more difficult claim to support
empirically, so we leave its investigation to future work.
...
enough samples) even approaches a normal distribution. Paired difference tests
are also robust to the censoring caused by timeouts: upper-bounding the run time
...
Altogether TIP problem
by QuickSpec and IsaCoSy theory
