...
Unfortunately, uniform sampling of definitions gives rise to a lottery, where
for a large corpus and a fixed sample size, it becomes highly unlikely that a
sample will contain the particular definitions required by a theorem. This would
cause most samples to have an empty ground truth, and hence $0$ precision and
...
definitions as our sample, padding uniformly up to the required size (we ignore
theorems which reference more definitions than our sample size).

We form the ground truth as above: selecting all applicable theorems from the
corpus, which is now guaranteed to be non-empty.
...
definitions into the required input format, which depends on the particular
tool being studied, then we time the execution, with a cutoff after e.g. 1 hour.

In our experiments we have found that memory usage also plays an important role,
but rather than introducing an extra dimension we can account for this by simply
allowing the system
