...
theorems, for even sample sizes up to 20. We chose 30 samples for each size, but
due to the requirement for samples to admit at least one theorem, only 29 unique
samples were obtained for size 2. A timeout of 5 minutes was imposed, since
preliminary experiments showed that most QuickSpec runs either finish before
this time, or still do not finish after an hour. The running times are shown in
figure~\ref{figure:quickspec_runtimes}.

The only errors we encountered were timeouts (shown in red), and these occurred
for all sample sizes. The running times cluster, with many close to zero and
many exceeding the timeout, but with few in between. There is a clear trend
towards longer times and more timeouts as the sample size increases, such that
most samples of size 20 timed out.

Most runs finished quickly and generated few conjectures, with the most being
116 conjectures from a sample of size 16.
