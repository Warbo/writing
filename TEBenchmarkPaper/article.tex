...
``typical'' input of that size. Since our methodology measures performance over
many samples, we can also compute the \emph{spread} of our data, for example the
inter-quartile range. This is not possible using the one-off measurements common
in prior evaluation methods.

There are two ways to average the measured precision and (separately) recall for
samples of a particular size. Since these quantities are ratios, their mean
values are known as the \emph{average of the ratios} (AOR), which we can define
for $S$ samples indexed by $1 \leq i \leq S$ as follows:
...
We interpret these as the \emph{expected} values of precision and recall for
samples of this size, i.e. the expected quality of the \emph{set} of conjectures
that will be generated from such a sample.

The alternative is to calculate \emph{pooled} averages, by combining the
(multi)sets from each sample before taking the ratios. If we multiply the
numerator and denominator by a factor $\frac{1}{S}$ we find that this is
equivalent to using the mean set sizes, hence this is known as the
...
each set equally. This weighting gives us expected values of \emph{conjectures}
rather than sets: the ROA for precision is the expected chance of a generated
conjecture being interesting; the ROA for recall is the expected chance that a
ground truth property will be generated. Note that both of these averaging
methods deal with \emph{fractions}: we may also want to know the \emph{absolute}
number of conjectures to expect, which is the mean output size
$\overline{\#\text{generated}}$.

If we instead want to combine precision values \emph{with} recall values (either
individually or pooled) we can calculate their \emph{F-score}, defined as:
...
For simplicity, we suggest comparing quality by pooling results from all samples
where both tools finished successfully. Recall can be compared using the
standard McNemar test for paired samples~\cite{mcnemar1947note}: each
...
names as the ground truth, rather than some lossy variant.
