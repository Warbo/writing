...
% FIXME: Strengthen motivation:
%  - Automated Scientific Discovery (e.g. Dundee drug discovery "centaur" story)
%  - "Mixed initiative" (social machine?); maybe Kasparov
...
One way to reduce these hurdles is to increase automation, which in the case of
software verification can benefit from the introduction of conjecture generation
tools~\cite{Claessen.Johansson.Rosen.ea:2013}. Another approach is to tailor the
division of labour between humans and machines such that each is playing to
their strengths, which has been referred to as ``centaur
teams''~\cite{harari2017reboot, davenport2015beyond}. In this respect a
conjecture generating tool can search a much larger space of relationships than
a human, whilst leaving the ultimate judgement of which to investigate in more
detail to the user.
...
example searching for new drugs. The major difference between scientific and
...
We limit ourselves to applications in mathematics, but even here it is difficult
...
% FIXME: Perhaps mention examples like "plus(x, 0) = 0" as motivating examples
% of things which are "simple" but which proof assistants still don't do
% automatically (reference Moa's use of TE for background lemma generation)
...
% FIXME: We jump from conjectures to theorems. Maybe to prevent confusion we
% should talk more generally about "interesting properties" of a formal system,
% and use that as motivation for conjecturing and proving?
...
% FIXME: How are these referred to/considered in the paper?
\begin{enumerate}
...
definitions shown in figure~\ref{figure:list_theory}), %FIXME: add page ref
...
Despite these limitations we believe that this is a viable approach to drawing
...
\item Since it will act as our ground truth, the problem set should
  ...
  unique definitions. % FIXME: Reword: make clear that non-membership means
  % uninteresting, and hence the things we *don't* include are just as important
  % as the things we do
\item The problem set should be as large as possible, for robustness of the
  resulting statistics. % FIXME: Prioritising quantity over quality?
\end{itemize}
...
% FIXME: Add section references to the braces
% FIXME: Maybe rethink "analysis" bit; maybe add summary and comparison
% references (although maybe not as full blown steps)
\begin{figure}
...
% FIXME: Maybe use (red?) crosses rather than red dots? More useful for black and white
...
\subsection{TIP} % FIXME: Expand acronym
...
% FIXME: The designers must decide on, not the system
...
being, although we would be delighted if proved wrong. % FIXME: Is there a solution? Mention that this is common to most tests
...
% FIXME: Reword into a short intro, e.g. "There is a long history of automating
% mathematics, going back to... A recurring theme is separation between the
% efforts of pure mathematicians and those wanting practical engineering
% applications....
...
% FIXME: Cut this down a lot. Currently it's not even an ATF section, it's just
% an AM section. We should maybe remove its status as a section, and instead
% just remark on the influence of AM on the field
...
The phrase Mathematical Theory Exploration (MTE) is a recent term % FIXME: When?
which seems to have originated with Buchberger and colleagues (see, % FIXME: informal
...
% IsaScheme FIXME: Maybe strip down this description? We could use prose instead,
% like "schemes capture common patterns such as iteration, which can be
% instantiated with an addition function to produce multiplication, or with
% multiplication to produce exponentiation.
