...
Figure~\ref{figure:list_theory}, we may want to find properties (such as
...
uses of this library, for example by rewriting expressions into equivalent forms
which require less time, memory, network usage, etc. For example, our theory of
lists turns out to satisfy the following invariant:
...
least), giving rise to the ``map/reduce'' paradigm for parallel programming.
Such use cases require the ability to \emph{discover} properties about some
...
are interesting properties of those definitions.
...
$\S$\ref{section:prep}: each benchmark problem has standalone type and function
