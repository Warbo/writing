...
\begin{figure}
  ...
  \caption{Running times of QuickSpec on theories sampled from TIP. Each point
    ...
    quartiles near zero or timeout.}
...
preliminary experiments showed that most runs (around $\frac{4}{5}$) either
finish before this time, or still do not finish after an hour. The results are
shown in figure~\ref{figure:quickspec_results}.

The only errors we encountered were timeouts, and these occurred for all sample
sizes. Whilst the median runtimes are mostly in the range of a few seconds, the
variance increased dramatically towards size 10, and $\frac{1}{4}$ of sample
size 13 timed out.

Most runs finished quickly and generated few conjectures, with the most being
131 conjectures from a sample of size 20. Sizes 15 to 20 show a slight but
consistent reduction in runtimes, which is likely an artefact of our testing
setup.
...
\texttt{lambda} functions (which QuickSpec cannot synthesise) were replaced with
...
If ATE matures to the point of providing \emph{novel} insights, even in
...
Our benchmarking approach only applies to systems which act in ``batch mode'',
i.e. those which choose to halt after emitting some output. Whilst all of the
systems we have encountered are of this form, an analogous benchmark may be
desirable for systems which instead run continuously or interactively (perhaps
akin to McCarthy's ``advice taker''%FIXME~\cite{}.
