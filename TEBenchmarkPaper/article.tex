...
and provide those definitions as our MTE input. We generate a corresponding
ground truth by selecting those theorems from the corpus which only contain
references to the sampled definitions.
...
the definitions required by a theorem. The majority of such samples would hence
have an empty ground truth, and thus $0$ precision and undefined recall
\emph{independent} of what the MTE system produces! This is clearly undesirable
...
point), and hence have a meaningful precision and recall.
...
\begin{sloppypar}
  ...
  forms instead of the raw constructors and destructors, and include these
  functions in our samples in lieu of the raw expressions. Note that these
  destructor wrappers are \emph{partial} functions (e.g.
  \texttt{destructor-head} and \texttt{destructor-tail} are undefined for the
  input \texttt{Nil}), which complicates their translation to proof assistants
  like Isabelle.
\end{sloppypar}
...
``user-level'' \texttt{declare-datatypes} and \texttt{define-fun} mechanisms.
...
and we choose to only sample from those 182 which occur in at least one theorem
statement (this removes ambiguity about which \emph{definitions} count as
``interesting'' and which are just ``implementation details'' for other
definitions).

TIP includes a tool to translate its definitions into Haskell and Isabelle,
including suitable comparison functions and random data generators. We translate
all 269 unique definitions into a single module/theory which is imported on each
run of the MTE tools (although only those functions which appear in the current
sample are actually explored).
...
\begin{figure}
  ...
  \caption{Running times of QuickSpec on theories sampled from TIP. Each point
    ...
    shown in red. Some points are shifted horizontally to prevent overlaps.
    Standard box plots are included, with whiskers showing points at
...
We applied our sampling methodology to the 182 definitions which appear in TIP
theorems, for even sample sizes up to 20. We chose 30 samples for each size,
although one sample of size 2 was discarded as it was a duplicate. A timeout of
5 minutes was imposed, since preliminary experiments showed most QuickSpec runs
to either finish before this time, or still be running after an hour. The
running times are shown in figure~\ref{figure:quickspec_runtimes}.

The only errors we encountered were timeouts (shown in red), which occurred for
all sample sizes but are more common for larger samples (over half the runs for
sample size 20 timed out). Runs which did not time out mostly finished within 30
seconds, generating few or no conjectures; those taking longer generated more
conjectures, with the most being 116 conjectures from a sample of size 16.
...
conjecture is drawn from a pool of wanted/unwanted results (in the case of
precision) or found/missed ground truths (in the case of recall), with the mean
of these distributions estimating the proportions of each pool's split.
...
above size 6 exceeded 50\%.

Recall shows a slight downward trend but is mostly flat, averaging between 20\%
and 50\%. This is expected, since adding more definitions to a QuickSpec
