...
These rose trees are then turned into matrices, as shown in Figure \ref{fig:matrixexample}. Each row $i$ of the matrix contains the features at depth $i$ in the rose tree, with any required padding added on the right. These matrices are either padded with $0$ or truncated, to fit a fixed size $r \times c$.
