...
mpl.rcParams.update({
    ...
    'axes.linewidth' : 0.1,  # Avoid obscuring error bars and data points
...
succeeded = lambda agg: lambda i: agg['success'][i]
...
def plotTime(system, agg):
    ...
    percentile = lambda p: [np.percentile([t for i,t in enumerate(agg['time']) \
                                           if agg['size'][i] == size],
                                          p) \
                            for size in sizes]
    p25 = percentile(25)
