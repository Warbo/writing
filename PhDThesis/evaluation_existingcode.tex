To see how our recurrent clustering implementation performs outside our own
benchmark we have also applied it to existing code, with mixed results. Unlike
our benchmark, these scenarios have no ground truth or measure of success to
compare against (there is no objectively correct way to compare expressions).
Instead, we provide a qualitative overview of the more interesting
characteristics noticed in these explorations.
...
For a more realistic test we also applied recurrent clustering to a variety of
the most-downloaded packages from \hackage{} (as of 2015-10-30), including
...
It is currently infeasible to extend these clustering applications into full
theory exploration runs, due to the use of constraints like \quickcheck{}'s
\hs{Arbitrary} type class (or \leancheck{}'s , in the case of \speculate{}) to
generate suitable values for instantiating variables. Whilst we can
...
measuring the direct impact on \quickspec{} performance on real code examples,
either directly by exploring the sub-sets identified through recurrent
clustering or indirectly by comparing the equations generated by a full
brute-force search to our recurrent clusters.
