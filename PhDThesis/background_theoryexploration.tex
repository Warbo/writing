...
In this work we consider the application of \emph{(automated) theory
  exploration} (TE) to generate conjectures about code\footnote{In principle TE
  can be used with any formal system, but appropriate methods can differ
  according to the underlying logic, etc. that is in use. We focus on Haskell
  code to anchor these variables, and to ensure our results have immediate
  utility.}. Existing tools are able to \emph{prove} those conjectures, and
hence output \emph{novel} theorems without guidance from the user. The method of
conjecture generation is a key characteristic of any theory exploration system,
although all existing implementations rely on brute force enumeration to some
degree.
