...
even (S n) = odd n
  ...
  \caption{Haskell code, defining datatypes and functions involving them (note
    ...
    with elements of type \hs{t}. Functions are called using whitespace, e.g.
    \hs{f x}, and may be defined by pattern-matching (case analysis) on their
    arguments. \hs{A :: B} is an optional type annotation, stating that value
    \hs{A} has type \hs{B}. \hs{A -> B} is the type of functions from \hs{A} to
    \hs{B}.}
