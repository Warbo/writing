...
Functional programming is important (motivation)

introduce running example

Haskell is a mainstream FP language (motivation)

Important to test (example)

Tests are not sufficient (example)
...
The full Haskell language is rather complex, so we focus instead on \emph{Core}, an intermediate representation of the Glasgow Haskell Compiler (GHC) based on \fc. For a full treatment of \fc, and its use in GHC, see \citep[Appendix C]{sulzmann2007system}. The sub-set of Core we consider is shown in figure \ref{coresyntax}; compared to the full language \footnote{The current version being present in GHC 7.10.2}, we erase types, use a custom  various forms of literal simplifications include perform type erasure

We follow it is capable of expressing just as is capable of used as an intermediate which the Glasgow Haskell Compiler (GHC) target by
