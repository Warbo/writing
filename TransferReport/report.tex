...
\begin{itemize}
  ...
  \item{Counterexample Generation}: As mentioned in \S \ref{tests}, there are many Haskell property checkers, which differ mostly in the way they generate values for testing. Since our theory exploration tools are built on these property checkers, we rely on the existence of value generators for all of the types in our theory; yet existing approaches to deriving such generators (e.g. as found in the \texttt{derive} package) rely on choose between constructors, recursively. As demonstrated in \S \ref{haskell}, a type's constructors may not be exported, in which case these methods fail; not to mention the exponential complexity of such recursive branching. It seems that a database of inspectable Haskell functions may be useful for solving this problem, although we have not yet performed experiments in this area.
\end{itemize}
