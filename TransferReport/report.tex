...
\begin{figure}
  ...
  \begin{tikzpicture}[node distance = 2cm, auto]
    % Place nodes
    \node [block] (cf) {Concept Formation};
    \node [block, below of=cf] (cg) {Conjecture Generation};
    \node [block, below of=cg] (tp) {Theorem Proving};
    ...
    (cf.north west) -- (tp.south west) node [black,midway,xshift=-1.5cm]
    {\footnotesize ITP};

    \draw [decorate,decoration={brace,amplitude=10pt}]
    (tp.north east) -- (tp.south east) node [black,midway,xshift=0.3cm]
    {\footnotesize ATP};

    \draw [decorate,decoration={brace,amplitude=10pt}, xshift=5cm]
    ($ (cg.north east) + (1,0) $) -- ($ (tp.south east) + (1,0) $)
    node [black,midway, xshift=0.6cm] {\footnotesize Theory Exploration};
  \end{tikzpicture}
